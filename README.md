...
이 저장소 자체는 **템플릿을 관리하는 곳**이라 자기 작업 기록(`.ai/work-result/`, `.ai/issues/`)을
남기지 않는다. 폴더와 템플릿 파일은 가져다 쓸 프로젝트를 위한 구성물이다.
...
`.ai/design/`은 **미리 만들지 않는다** — 컴포넌트가 실제로 생겼고 구조를 말로 설명해야 할 때
파일을 추가한다. 인터페이스 계약 등 그 밖의 문서도 필요해질 때 `.ai/` 아래에 두고
`.ai/README.md`에 등록한다.
...
| `{{component}}` | 설계 문서 파일명 (`.ai/design/`) | `server.md`, `web.md` |

   **(나) 문서를 쓸 때 채우는 것** — 위 표에 **없는** 나머지 `{{...}}` 전부.
   `{{왜 이 결정이 필요했는가…}}` 같은 안내문은 물론, `{{기능 1}}`·`{{하고 싶은 것}}` 처럼
   짧은 것도 여기 속한다. 백로그·상태·설계·ADR·이슈·작업 결과 문서를 **쓸 때마다** 그 자리에
   내용을 채우므로 도입 시점에 일괄 치환하지 않는다. 템플릿 파일(`0000-*`)에는 그대로 둔다.

   > 판단 기준: **저장소 전체에서 항상 같은 값이면 (가), 문서·항목마다 값이 달라지면 (나)** 다.

3. **AGENTS.md를 먼저 채운다.** 나머지 파일은 전부 AGENTS.md를 가리키는 구조이므로,
   3~5절(아키텍처 원칙 / 저장소 배치 / 코딩 규칙)만 확정해도 에이전트가 일할 수 있다.
   특히 **5절의 빌드/실행 표**는 에이전트가 가장 먼저 찾는 정보이니 비워 두지 않는다.
   6절 이하는 프로젝트가 자라면서 채우고, 해당 없는 절은 지우지 말고 "해당 없음"으로 남긴다.

   AGENTS.md에는 **규칙만 둔다**(ADR 0007). 이 프로젝트가 무엇을 만드는지는 프로젝트
   `README.md`에, 무엇을 만들기로 했는지(범위)는 `.ai/status.md`의 구현 현황 표에 적는다.
...
6. **마무리.**
   - **템플릿의 결정 기록을 지운다** — `.ai/adr/`의 `0001`~`0006`과 인덱스 표의 해당 행.
     이 ADR들은 *템플릿이 왜 이런 모양인지*에 대한 기록이라 새 프로젝트의 결정 기록과 섞이면
     안 된다. 템플릿 파일(`0000-*`)과 폴더는 남긴다.
     (규칙 자체는 `AGENTS.md`와 `.ai/README.md`에 이미 녹아 있으므로, ADR을 지워도 규칙은 남는다.)
...
- **갱신 방식이 다른 문서를 섞지 않는다.** 덮어쓰는 문서(백로그·상태·설계)와 추가만 하는
  문서(ADR·작업 결과)를 한 파일에 두면, 최신 상태를 알려면 과거를 걷어내야 하고 과거를
  보려면 최신본이 이미 덮어쓴 뒤다. 위의 역할표가 이 원칙 하나에서 나온다.
- **규칙 문서에는 규칙만 둔다.** 아이디어는 `.ai/backlog.md`, 하기로 정한 범위와 그 진행은
  `.ai/status.md`의 구현 현황 표 하나가 담는다. 범위 목록을 규칙 문서에 두면 범위를 정할
  때마다 규칙 문서가 흔들리고, 같은 항목을 상태 문서와 두 번 적게 된다.
