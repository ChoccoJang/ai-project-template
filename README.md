...
| `check_doc_index.py`<br>(+`pr_field.py`) | 파일을 만들고 **인덱스에 등록하지 않은** 실수, 한글 파일명, 번호 중복, `Phase`·`PR`·`갱신일` 필드 누락·공백, 상태 어휘 오타, 인덱스 표와 파일 머리말의 상태 불일치, **없는 절을 가리키는 "N절" 참조**, 작업 결과 문서 `PR` 값이 **링크 · `없음(사유)` · `미정`이 아닌 것**(CI는 `--merge-gate`로 `미정`도, PR이 새로 추가한 문서라면 `--added`로 `없음(...)`도 실패시킨다. 뒤늦은 기록의 탈출구 `없음(직접 push, abc1234)`는 **그 커밋이 이 PR 전에 이미 있던 것이어야** 통과한다) |
...
`check_doc_index.py`에는 옵션이 둘 있다. `--merge-gate`를 주면 작업 결과 문서의
`PR: 미정`을 실패로 본다. **CI는 항상 켜고**, 위 로컬 명령은 끈 채로 돌린다 — 작업 중에는
아직 PR이 없어 `미정`이 정상이기 때문이다
([`.ai/work-result/README.md`](./.ai/work-result/README.md)의 `PR` 필드 채우는 순서).
CI와 같은 상태를 로컬에서 보려면 같은 옵션을 붙인다. 옵션 없이 돌려도 `PR: 미정`은
**경고로 찍는다** — 로컬 초록을 "다 됐다"로 오해하지 않게 하기 위해서다.

CI는 여기에 더해, PR이 **새로 추가한** 작업 결과 문서를 `--added`로 넘겨 `PR: 없음(...)`도
실패시킨다 — PR로 올라오는 변경이 "PR 없음"이라 적고 있다는 뜻이기 때문이다.
