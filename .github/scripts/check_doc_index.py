...
(`없음(직접 push, abc1234)`). 그 해시가 **실제로 있는 커밋인지 확인하고**, `--base`를 함께
...
# 같은 폴더의 모듈이다 — 스크립트로 직접 실행하므로 그 폴더가 import 경로에 들어온다.
from pr_field import Run, check_pr_field
...
def check_section(section: Section, run: Run) -> list[str]:
    problems: list[str] = []
    directory = run.root / section.directory
    ...
    for path in sorted(directory.glob("*.md")):
        ...
        for name_ in section.extra_fields:
            ...
            if not value:
                ...
                problems.extend(check_pr_field(rel, value, run, rel in run.added))
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        default="",
        metavar="커밋",
        help="PR의 base 커밋. 뒤늦은 기록이 가리키는 커밋이 그 전에 이미 있던 것인지 본다",
    )
    parser.add_argument(
        "--added",
        default="",
